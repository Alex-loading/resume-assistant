...
3. 打开「开发者模式」→「加载已解压的扩展程序」，选择本项目的 **`extension` 文件夹**。也可解压 `dist/简历匣-v0.2.2.zip`，选择其中的 `resume-assistant` 文件夹。
...
5. 在「网页填写」页，下拉框默认仅列出候选资料；候选不合适时，点击「从所有字段中选择」，即可查看全部资料，其中候选仍排在前面。预览后点击「填入当前输入框」。

有多条教育或实习经历时，侧栏会高亮多个候选，供你选择正确记录。无匹配时下拉框直接显示全部资料。展开全部资料后可点击「返回候选字段」；切换输入框或简历版本，会重新优先显示当前候选。网页已有内容默认保留，明确开启「允许覆盖」后才可覆盖；切换输入框会重置覆盖开关。
...
- 个人信息、教育、实习/工作、项目、技能、个人介绍、荣誉。
- 「个人信息」支持选填身份证号，按文本保存（保留末尾的 X），可快捷复制、匹配和填写身份证号输入框。
- 「常用长文本」中的「个人介绍」支持多行文本，可匹配个人介绍、自我介绍、自我评价等网页字段；原有「自我介绍」内容保留，无需重新录入。
